...
def _data_unit_test(filepath_with_name):
    # The format of the version 2 of the symmetric dataset (symmetric_v0.2) isn't fully documented, and seems to have
    # some irrelevant fields, so here we just
    # check that the duplicated fields can in fact be ignored.
    line_id = 0
    with codecs.open(filepath_with_name, encoding="utf-8") as f:
        for line in f:
            if line_id % 1000 == 0:
            ...
            line = line.strip()
            data = json.loads(line)
            assert data["claim"] == data["sentence2"]
            assert data["evidence"] == data["sentence1"]
            assert data["gold_label"] == data["label"]
            assert len(data["evid"]) == 1
            assert data["evid"][0][0] == data["evidence"]
            assert data["evid"][0][1] == 1.0
...
def get_symmetric_data(filepath_with_name, tokenizer):
    ...
    with codecs.open(filepath_with_name, encoding="utf-8") as f:
        for line in f:
            ...
            assert data["evidence"] == data["sentence1"]
            assert data["gold_label"] == data["label"]
            assert data["label"] in ["SUPPORTS", "REFUTES"]
            assert len(data["evid"]) == 1
            assert data["evid"][0][0] == data["evidence"]
            assert data["evid"][0][1] == 1.0
            id = int(data["id"])
    ...
    for id in symmetric_data_grouped_ids_dict:
        assert len(symmetric_data_grouped_ids_dict[id]) == 4, f"ERROR: The data is expected to have a total of " \
                                                              f"4 instances associated with each base claim id."
...
def main(arguments):
    ...
    _data_unit_test(args.input_symmetric_jsonl_file)
