# -*- coding: utf-8 -*-
"""
This is similar to fever_symmetric_data_preprocess.py and fever_symmetric_data_preprocess_original_format.py, but
this is intended for the train and dev set in https://github.com/TalSchuster/FeverSymmetric. Note that these are the
original FEVER train and public dev sets, but limited to single evidence sets, and with claims duplicated to cover
all single evidence sets (to create unique claim-evidence pairs).

Note that the train set has unverifiable claims, which we ignore, since the symmetric dev (and reprocessed
original dev) only consider supports/refutes.
"""
...
def read_jsonl_with_symmetric_filtering(filepath_with_name, tokenizer, titles_dict, original_titles_dict,
    ...
    with codecs.open(filepath_with_name, encoding="utf-8") as f:
        for line in f:
            ...
            if parent_id in symmetric_data_grouped_ids_dict:
                for id in symmetric_data_grouped_ids_dict[parent_id]:
                    ...
                    if claim_diff.ratio() < 1.0:
                        ...
                        if claim_diff.ratio() < .95:
                            print(f"\tLOW CLAIM RATIO: POSSIBLE MISMATCH")
                    ...
                    # The train set has unverifiable claims, which we ignore in the symmetric analysis
                    ...
                    if verifiable == "VERIFIABLE":
                        ...
                        if wiki_diff_ratio < 1.0:
                            ...
                            print(
                                f"\tSymmetric wiki: {' '.join(symmetric_data_dict[id][1])}")
                            print(f"\tWiki difference ratio: {wiki_diff_ratio}")
                            if wiki_diff_ratio < .95:
                                print(f"\tLOW WIKI RATIO: POSSIBLE MISMATCH")
...
def get_claim_to_titles_lines_with_symmetric_filtering(symmetric_data_dict, symmetric_data_grouped_ids_dict,
    ...
    for parent_id in symmetric_data_grouped_ids_dict:
        for id in symmetric_data_grouped_ids_dict[parent_id]:
            # only consider SUPPORTS/REFUTES
            symmetric_label = symmetric_data_dict[id][2]
            if symmetric_label in ["SUPPORTS", "REFUTES"]:
                symmetric_claim = " ".join(symmetric_data_dict[id][0])
                symmetric_evidence_without_title = " ".join(symmetric_data_dict[id][1])

                # NOTE: unlike the re-annotated, here a single claim may be matched to evidence with varying titles
                ...
                symmetric_rematched_title = id_to_symmetric_title_and_sentid[id][0]
                symmetric_rematched_sent_id = id_to_symmetric_title_and_sentid[id][1]
...
def get_symmetric_data(filepath_with_name, tokenizer):
    ...
    # Note that this differs from get_symmetric_data() in fever_symmetric_data_preprocess_original_format.py in that
    # we need to create a unique 'id' for each instance, since some claims are duplicated (due to covering the
    # evidence sets by creating new claim-evidence pairs). For simplicity, we otherwise aim to use the same data
    # structures.
    ...
    with codecs.open(filepath_with_name, encoding="utf-8") as f:
        for line in f:
            ...
            # in this case, we create a unique id simply by appending the line id to the end of the string
            # representation of the original id, separated by '.' with a final 1 digit for uniqueness
            # in float conversion
            parent_id = int(data["id"])
            id_string = str(data["id"]) + "." + str(line_id-1) + "1"
            id = float(id_string)  # id is now a float

            claim, _ = detokenize_symmetric_data_text_string(data["claim"], md, tokenizer)
            evidence, _ = detokenize_symmetric_data_text_string(data["evidence"], md, tokenizer)

            assert id not in symmetric_data_dict
            symmetric_data_dict[id] = [claim, evidence, data["gold_label"]]
